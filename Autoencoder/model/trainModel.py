...
class TrainModel(BaseModel):
    ...
    def loss(self, predictions, labels, num_labels):
        ''' Computes Mean Squared Error loss between the input and output of the network.

          predictions: autoencoder's predictions
          labels: input values of the autoencoder which serve as labels at the same time
          num_labels: number of labels !=0 in the data set to compute the mean
    ...
    def validation_loss(self, x_train, x_test):
        ''' Computes the loss during the validation time.
        ...
        mask = tf.where(tf.equal(x_test, 0.0), tf.zeros_like(x_test),
                        x_test)  # identify the zero values in the test set
        ...
        MSE_loss = self.loss(outputs, x_test, num_test_labels)
    ...
    def train(self, x):
        '''Optimization of the network parameter through stochastic gradient descent.
            ...
            @param x: input values for the autoencoder.
        ...
        MSE_loss = self.loss(outputs, x, num_train_labels)
