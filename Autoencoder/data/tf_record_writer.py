...
path = os.path.join(ROOT_DIR,"tf_records")
try:
    os.mkdir(path)
...
def main():
    ''' Writes the .csv training and testing data into binary TF_Records.'''

    SAMPLES_PER_FILES = 100

    training_set, test_set = get_dataset_1M()

    for data_set, name, dir_ in zip([training_set, test_set], ['train', 'test'],
                                    [TF_RECORD_TRAIN_PATH, TF_RECORD_TEST_PATH]):
